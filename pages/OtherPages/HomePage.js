      ...
      <View>
        <View className="absolute w-full">
          ...
          <View
            ...
            className=" bg-black -z-30 opacity-20"
          />
        </View>
        ...
        <View className="p-4 pt-10">
          <View className="flex-row justify-between items-center">
            ...
            <TouchableOpacity
              ...
              className="bg-white h-8 px-3 rounded-full flex-row justify-center items-center">
              <Feather name="shopping-bag" color="black" size={20} />
              <Text
                style={{fontFamily: 'Poppins-Medium'}}
                className="text-black mt-1 mx-1">
                {CartData < 10 && CartData > 0 ? '0' + CartData : CartData}
              </Text>
            </TouchableOpacity>
          </View>
        </View>
        ...
        <View className="px-4 flex-col">
          <Text
            style={{marginTop: height * 0.2, fontFamily: 'Poppins-SemiBold'}}
        ...
        <ScrollView
          ...
          {ShoesData.slice(0, 10).map(
            (item, index) =>
              !Loading && (
                <TouchableOpacity
                  key={index}
                  onPress={() => navigation.navigate('Shoes', {item: item})}
                  className="bg-white h-72 w-48 p-1.5 m-2 rounded-xl shadow-md">
                  <Image
                  ...
                  <Text
                    style={{fontFamily: 'Poppins-SemiBold'}}
                    className="text-xl text-black ml-2 mt-4">
                    &#8377;{(item.allProducts[0]?.productPrice * 80).toFixed(2)}
                  </Text>
                  <Text
                    style={{fontFamily: 'Poppins-SemiBold'}}
                    className="text-black ml-2 mt-2">
                    {item.productName}
                  </Text>
                  <Text
                    style={{fontFamily: 'Poppins-SemiBold'}}
