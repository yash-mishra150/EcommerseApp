...
const RegisterPage = () => {
    ...
    const handleRegister = () => {
        ...
        setProgress(0.5);
        setStep(1); // Move to ManualRegister
    };

    return (
        <View style={{ flex: 1, padding: 16, paddingTop: 32, backgroundColor: 'white' }}>
            <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                <TouchableOpacity onPress={() => navigation.navigate('Starter')}>
                    <Icon name='arrow-left' size={25} color='black' />
                </TouchableOpacity>
                <Text style={{ fontFamily: 'Poppins-Medium', marginLeft: height * 0.03, marginTop: 8, fontSize: 18, color: 'black' }}>
                    Create an Account
                </Text>
            </View>

            <View style={{ marginTop: height * 0.05, width: '100%' }}>
                <ProgressBar progress={progress} color='black' style={{backgroundColor: '#d3d3d3'}} />
            </View>

            <View style={{ marginTop: height * 0.05, width: '100%', alignItems: 'center' }}>
                {step === 1 && <OTPVerify email={email} handle={handleOTPVerify} />}
                {step === 0 && <RegisterForm handle={() => handleRegister()} setemail={setemail}/>}
                {step === 2 && <ManualVerify />}
            </View>
        </View>
    );
